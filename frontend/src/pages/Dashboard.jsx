...
import {
  ...
  Tag, AlignLeft, PenLine, Link, StickyNote, Plus, ArrowRight
} from 'lucide-react';
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.855rem' }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${token.border}` }}>
              {[['No', '3rem'], ['PT', '4rem'], ['Nomor Dokumen', 'auto'], ['Judul', 'auto'], ['User', 'auto'], ['Tanggal', '7rem'], ['', '12rem']].map(([h, w]) => (
          ...
          <tbody>
            {pageData.map((doc, idx) => (
              ...
              >
                ...
                <td style={{ padding: '0.8rem 1rem', color: token.text, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.judul_dokumen || <span style={{ color: token.muted }}>—</span>}</td>
                <td style={{ padding: '0.8rem 1rem', color: token.text }}>{doc.user_name}</td>
                <td style={{ padding: '0.8rem 1rem', color: token.muted, whiteSpace: 'nowrap' }}>{doc.doc_date}</td>
                <td style={{ padding: '0.8rem 1rem' }}>
                  <div style={{ display: 'flex', gap: '0.35rem' }}>
                    <Btn variant="soft"   style={{ padding: '0.28rem 0.6rem', fontSize: '0.73rem' }} onClick={() => startDuplicate(doc)}><Copy size={12} /> Duplikat</Btn>
                    <Btn variant="ghost"  style={{ padding: '0.28rem 0.6rem', fontSize: '0.73rem' }} onClick={() => startEdit(doc)}><Edit size={12} /> Edit</Btn>
                    <Btn variant="success" style={{ padding: '0.28rem 0.6rem', fontSize: '0.73rem' }} onClick={() => hDownload(doc)}><Download size={12} /> Docx</Btn>
                  </div>
                </td>
              </tr>
            ))}
